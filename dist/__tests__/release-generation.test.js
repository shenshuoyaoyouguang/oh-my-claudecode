...
describe('release generation', () => {
    ...
    it('enforces the release publication boundary around one exact archive', () => {
        ...
        expect(workflow).toContain('- name: Verify plugin shipping surface\n        run: npm run plugin:shipping:verify\n\n      - name: Build');
        ...
        expect(workflow).toContain('if [ "$FALLBACK_BACKOFF_SECONDS" -gt 60 ]; then\n                FALLBACK_BACKOFF_SECONDS=60\n              fi');
        ...
        expect(releaseJob).toContain('if [ "$BACKOFF_SECONDS" -gt 60 ]; then\n                BACKOFF_SECONDS=60\n              fi');
        const propagationLimits = [...workflow.matchAll(/MAX(?:_FALLBACK)?_PROPAGATION_ATTEMPTS=(\d+)/g)].map((match) => match[1]);
